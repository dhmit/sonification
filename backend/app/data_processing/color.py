...
def generate_samples(colors, sine_only=False):
    '''
    ...
    for hsv in colors:
        ...
        if sine_only:
            sample = synths.generate_sine_wave_with_envelope(freq, 1)
        else:
...
def get_harmonic_weights(value):
    ...
    '''
    sine_harmonics = np.array([1, 0, 0, 0, 0, 0, 0])
    square_harmonics = np.array([0.6366, 0, -0.2122, 0, 0.1273, 0, -0.0909])
    triangle_harmonics = np.array([0.8106, 0, 0.0901, 0, 0.324, 0, 0.0165])
    sawtooth_harmonics = np.array([0.6366, -0.3183, 0.2122, -0.1592, 0.1273, -0.1061, 0.0909])
